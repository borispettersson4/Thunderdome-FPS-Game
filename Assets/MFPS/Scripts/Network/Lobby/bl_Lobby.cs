...
public class bl_Lobby : bl_PhotonHelper, IConnectionCallbacks, ILobbyCallbacks, IMatchmakingCallbacks
{
    ...
    [SerializeField] private Text MapLoadingTitle;
    [SerializeField] private Image MapLoadingBackground;
    ...
    public void ChangeGameMode(bool plus)
    {
        ...
        bool r = false;
#if ELIM
      if(GameModes[CurrentGameMode].m_GameMode == GameMode.ELIM) { r = true; }
#endif
#if BDGM
      if(GameModes[CurrentGameMode].m_GameMode == GameMode.SND) { r = true; }
#endif
    ...
    {
        var sceneName = (string)PhotonNetwork.CurrentRoom.CustomProperties[PropertiesKeys.SceneNameKey];
        var selectedLobby = new bl_GameData.SceneInfo();

        foreach (var scene in bl_GameData.Instance.AllScenes)
        {
            if (scene.RealSceneName == sceneName)
            {
                selectedLobby = scene;
                break;
            }
        ...
        MapLoadingBackground.sprite = selectedLobby.Preview;
        MapLoadingTitle.text = selectedLobby.ShowName;
