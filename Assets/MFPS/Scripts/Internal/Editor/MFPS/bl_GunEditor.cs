...
public class bl_GunEditor : Editor
{
    ...
    private void OnEnable()
    {
        ...
        if (playerSettings != null) { AimReference = playerSettings.m_hands.AimPositionReference; }
    ...
    void DrawGlobalSettings()
    {
        ...
        if (t == GunType.Machinegun || t == GunType.Pistol || t == GunType.Burst || t == GunType.Shotgun)
        {
        ...
        if (t == GunType.Shotgun)
        {
            EditorGUILayout.BeginHorizontal("box");
            int w = ((int)EditorGUIUtility.currentViewWidth / 3) - 25;
            script.isPumpAction = EditorGUILayout.ToggleLeft("Pump Action", script.isPumpAction, GUILayout.Width(w));
            GUI.enabled = true;
            EditorGUILayout.EndHorizontal();
        }
    ...
    void DrawAimSettings()
    {
        ...
        script.AimFog = EditorGUILayout.Slider("Aim FoV (Zoom)", script.AimFog, 0.0f, 179);
    ...
    void DrawAmmoSettings()
    {
        ...
        {
            script.SplitReloadAnimation = EditorGUILayout.ToggleLeft("Split Reload Animation", script.SplitReloadAnimation,EditorStyles.toolbarButton);
        }
    ...
    void DrawRecoil()
    {
        script.ShakeIntense = EditorGUILayout.Slider("Head Shake On Fire", script.ShakeIntense, 0.0f, 2.0f);
    ...
    {
        script.baseSpread = EditorGUILayout.Slider("Spread", script.baseSpread, 0.0f, 6);
        script.maxSpread = EditorGUILayout.Slider("Max Spread", script.maxSpread, 0, 6);
