...
public class bl_RoomSettings : bl_MonoBehaviour
{
    ...
    public override void OnUpdate()
    {
        if (m_GameMode == GameMode.FFA)
        {
            try
            {
                List<bl_PlayerScoreboardUI> scoreboard = new List<bl_PlayerScoreboardUI>(UIReferences.getRoomScoreBoard());
                scoreboard.Remove(scoreboard.Where(x=>x.GetName() == PhotonNetwork.LocalPlayer.NickName).FirstOrDefault());
                var playerFirst = scoreboard.Where(x => x.GetKills() == scoreboard.Max(v => v.GetKills())).FirstOrDefault();

                //Debug.Log("First : " + playerFirst.GetName() + " Kills : " + playerFirst.GetKills() + " | Second : " + playerSecond.GetName() + " Kills : " + playerSecond.GetKills());

                if (playerFirst.GetKills() > 0)
                {
                    UIReferences.PlayerUI.FFALocalPlayerScoreText.text = PhotonNetwork.LocalPlayer.GetKills().ToString();
                    UIReferences.PlayerUI.FFAFoePlayerText.text = playerFirst.GetName();
                    UIReferences.PlayerUI.FFAFoePlayerScoreText.text = playerFirst.GetKills().ToString();
                }
            }
    ...
    public void CheckScore()
    {
        ...
        {
            var scoreboard = UIReferences.getRoomScoreBoard();
            var playerFirst = scoreboard.Where(x => x.GetKills() == scoreboard.Max(v => v.GetKills())).FirstOrDefault();

            if (playerFirst.GetKills() >= MaxKills)
            {
                UIReferences.UpdateScoreboard();
                TimeManager.FinishRound();
            }
    ...
    public string GetWinnerName
    {
        get
        {
            if (!isOneTeamMode)
            {
                ...
                {
                    return bl_Localization.Instance.GetText(30).ToUpper();
                }
            }
            else
            {
