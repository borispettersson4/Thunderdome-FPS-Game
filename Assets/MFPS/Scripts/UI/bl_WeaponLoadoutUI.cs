...
public class bl_WeaponLoadoutUI : MonoBehaviour
{
    ...
    private void Awake()
    {
        ...
        IconsTxt = new Text[SlotsGroups.Length];
        for (int i = 0; i < SlotsGroups.Length; i++)
        {
            IconsImg[i] = SlotsGroups[i].GetComponentInChildren<Image>();
            IconsTxt[i] = SlotsGroups[i].GetComponentInChildren<Text>();
        }
        IconsImg[0].CrossFadeColor(Color.black, 0.1f, true, true);
        IconsTxt[0].CrossFadeColor(Color.black, 0.1f, true, true);
    }
    ...
    public void SetInitLoadout(List<bl_Gun> guns)
    {
        for (int i = 0; i < SlotsGroups.Length; i++)
        {
            ...
            IconsTxt[i].canvasRenderer.SetColor(Color.white);
            ...
            Text txt = SlotsGroups[i].GetComponentInChildren<Text>(false);
            txt.text = guns[i].Info.Name;
        }
        BackRect.position = SlotsGroups[0].position;
        current = 0;
        IconsImg[0].canvasRenderer.SetColor(Color.black);
        IconsTxt[0].canvasRenderer.SetColor(Color.black);
    }
    ...
    public void ReplaceSlot(int slot, bl_Gun newGun)
    {
        ...
        IconsTxt[slot].text = newGun.Info.Name;
    }
    ...
    IEnumerator ChangeSlot(int nextSlot)
    {
        ...
        IconsTxt[cacheActual].CrossFadeColor(Color.white, 0.3f, true, true);
        IconsTxt[nextSlot].CrossFadeColor(Color.black, 0.3f, true, true);
        while (d < 1)
        {
            d += Time.deltaTime * 7;
