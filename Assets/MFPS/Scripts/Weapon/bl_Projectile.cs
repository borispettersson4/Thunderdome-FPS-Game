...
public class bl_Projectile : MonoBehaviour
{
    ...
    void OnCollisionEnter(Collision enterObject)
    {
        if(Sticky)
        {
            ...
            joint.connectedBody = enterObject.rigidbody;
        }
            ...
            default:
                ...
                if (m_Type == ProjectileType.Grenade)
                {
                    ...
                    if (blast != null)
                    {
                        ...
                        if (AIFrom) { blast.AISetUp(AIViewID, t, BotName); Debug.Log("GRENADE ID : " + ID); }
                    }
                }
                ...
                else if (m_Type == ProjectileType.Grenade_Molotov)
                {
                    bl_Blast blast = e.GetComponentInChildren<bl_Blast>();
                    if (blast != null)
                    {
                        blast.isNetwork = isNetwork;
                        blast.WeaponID = ID;
                    ...
                    bl_DamageArea da = e.GetComponentInChildren<bl_DamageArea>();
                    da.SetInfo(AIFrom ? BotName : PhotonNetwork.NickName, isNetwork);
                }
