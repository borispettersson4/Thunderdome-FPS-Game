...
public class bl_GameFinish : bl_PhotonHelper
{
    ...
    public void CollectData()
    {
        ...
        if (!isOneTeamMode)
        {
            ...
            {
                if (PhotonNetwork.LocalPlayer.GetPlayerTeam() == Team.Delta) { winner = true; }
            }
            else if (bl_RoomSettings.Instance.Team_1_Score < bl_RoomSettings.Instance.Team_2_Score)
            {
                if (PhotonNetwork.LocalPlayer.GetPlayerTeam() == Team.Recon) { winner = true; }
            }
        }
        else
        {
            if (bl_RoomSettings.Instance.GetWinnerName == PhotonNetwork.NickName) { winner = true; }
        }
        ...
        TotalScoreText.text = string.Format("{0}\n<size=25>{1}</size>", tscore, bl_Localization.Instance.GetText(35).ToUpper());
#else
