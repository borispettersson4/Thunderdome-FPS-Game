...
public class bl_CustomizerManager : MonoBehaviour
{
    ...
    void Awake()
    {
        if (PlayerPrefs.HasKey(bl_CustomizerData.CustomizerWeaponKey))
        {
            Have_Name = true;
            Cryted_Name = PlayerPrefs.GetString(bl_CustomizerData.CustomizerWeaponKey);
    ...
    void InstanceCustomizers()
    {
        for (int i = 0; i < AllCustom.Count; i++)
        {
            ...
            g.GetComponent<bl_CustomizerInfoButton>().Init(AllCustom[i].WeaponName);
        }
    }
    ...
    public void Active_Custom(string t_active)
    {
        CurrentName = t_active;
