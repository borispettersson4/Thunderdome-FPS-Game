...
public class bl_GameData : ScriptableObject
{
   ...
   /* [Reorderable]*/
    ...
    public GunType GetWeaponType(string gunName)
    {
        GunType type = GunType.Machinegun;
        if (AllWeapons.Exists(x => x.Name == gunName))
        {
            type = AllWeapons.Find(x => x.Name == gunName).Type;
        }
        return type;
    }

    public GunType GetWeaponType(int ID)
    {
        if (ID < 0 || ID > AllWeapons.Count - 1)
            return AllWeapons[0].Type;

        return AllWeapons[ID].Type;
    }

    public bool isWeapon(string gunName)
    {
        bool cond = false;

        try
        {
            return AllWeapons.Any(x => x.Name == gunName);
        }catch{ }
