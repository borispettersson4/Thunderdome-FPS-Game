...
public class bl_UIReferences : bl_PhotonHelper,IInRoomCallbacks
{
    ...
    [SerializeField] private Text FFATeamName;
    ...
    [Header("Sprites")]
    public Sprite BotLevelIcon;

    [Header("Menu UI")]
    [SerializeField] private GameObject MenuGUIBackground;
    ...
    private Team lastPlayerTeam = Team.Delta;
    ...
    void SetUpUI()
    {
        ...
        FFATeamName.text = GetGameMode.GetName();
        if (PhotonNetwork.IsConnected)
        {
            int MaxKills = (int)PhotonNetwork.CurrentRoom.CustomProperties[PropertiesKeys.RoomMaxKills];
#if LOCALIZATION
        ...
        for (int i = 0; i < players.Length; i++)
        {
            if (players[i].GetPlayerTeam() != Team.None)
            {
                ...
                else
                {
                    ...
                    if (!isOneTeamMode)
                    {
                        tp = ((string)players[i].CustomProperties[PropertiesKeys.TeamKey] == Team.Delta.ToString()) ? Team1Panel : Team2Panel;
                    }
                    else
                    {
                        tp = TeamFFAPanel;
    ...
    void SortScoreboard()
    {
        ...
        else
        {
            ...
            {
                if (cachePlayerScoreboard[i].GetTeam() == Team.Delta)
                {
                    cachePlayerScoreboardSorted.Add(cachePlayerScoreboard[i]);
                }
                else if (cachePlayerScoreboard[i].GetTeam() == Team.Recon)
                {
    ...
    void FlipScoreScreenPositions(GameObject team1UI, GameObject team2UI)
    {
        var team1UIPos = team1UI.GetComponent<RectTransform>().transform.position;
        var team2UIPos = team2UI.GetComponent<RectTransform>().transform.position;

        team1UI.GetComponent<RectTransform>().transform.position = team2UIPos;
        team2UI.GetComponent<RectTransform>().transform.position = team1UIPos;
    ...
    {
        MenuGUIBackground.GetComponent<Image>().enabled = active;
        ScoreboardUI.SetActive(active);
        ClassesUI.SetActive(active);
        ...
        else
        {
            ...
            CancelInvoke("UpdateScoreboard");
#if PSELECTOR
            FindObjectOfType<bl_PlayerSelector>().isChangeOfTeam = false;
#endif
        }
        State = (active) ? RoomMenuState.Full : RoomMenuState.Hidden;
    }
    ...
    public void ActiveClasses()
    {
        ClassesUI.SetActive(true);
        OptionsUI.SetActive(false);
        ScoreboardUI.SetActive(false);
        State = RoomMenuState.Classes;
    }
...
#if LOCALIZATION
        FinalUIText.text = (style == RoundStyle.OneMacht) ? LocaleStrings[3] : LocaleStrings[4];
    ...
    {
#if PSELECTOR
        FindObjectOfType<bl_PlayerSelector>().isChangeOfTeam = true;
#endif
        FFAJoinButton.SetActive(false);
        FFAScoreboardRoot.SetActive(false);
        JoinBusttons.SetActive(true);
    ...
    {
        ScoreboardUI.SetActive(active);
        State = (active) ? RoomMenuState.Scoreboard : RoomMenuState.Hidden;
        if (active) { InvokeRepeating("UpdateScoreboard", 1, 1); } else { CancelInvoke("UpdateScoreboard"); }
    }

    public void ShowClasses(bool active)
    {
        ClassesUI.SetActive(active);
    }
    ...
    void OnPlayerSpawn()
    {
        ...
        var player = PhotonNetwork.LocalPlayer;
        if (player.GetPlayerTeam() != lastPlayerTeam)
        {
            FlipScoreScreenPositions(PlayerUI.Team1ScoreCountUI, PlayerUI.Team2ScoreCountUI);
            FlipScoreScreenPositions(PlayerUI.Team1ScoreBoardUI, PlayerUI.Team2ScoreBoardUI);
        }
        lastPlayerTeam = player.GetPlayerTeam();
    ...
    public bool isMenuActive
    {
        get
        {
            return !(State == RoomMenuState.Hidden);
        }
    }
    ...
    public List<bl_PlayerScoreboardUI> getRoomScoreBoard()
    {
        return cachePlayerScoreboard;
    }
}