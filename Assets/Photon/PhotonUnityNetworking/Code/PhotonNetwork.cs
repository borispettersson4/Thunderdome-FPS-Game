...
namespace Photon.Pun
{
    ...
    public static partial class PhotonNetwork
    {
        ...
        public static Player GetPlayerFromPlayerList(string name)
        {
                Room room = CurrentRoom;
                if (room != null)
                {
                    return room.Players.Values.Where(x => x.NickName == name).OrderBy((x) => x.ActorNumber).FirstOrDefault();
                }
                return null;
        }

        public static Player GetPlayerFromPlayerList(Player player)
        {
            Room room = CurrentRoom;
            if (room != null)
            {
                return room.Players.Values.Where(x => x.UserId == player.UserId).OrderBy((x) => x.ActorNumber).FirstOrDefault();
            }
            return null;
        }
