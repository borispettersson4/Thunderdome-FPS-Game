...
public class bl_KillFeed : bl_PhotonHelper
{
    ...
    /// </summary>
    public void OnKillFeed(string t_Killer, string t_Killed, string t_HowKill, string t_team, int t_GunID, int isHeadShot)
    {
        photonView.RPC("AddNewKillFeed", RpcTarget.All, t_Killer, t_Killed, t_HowKill, t_team.ToString(), t_GunID, isHeadShot);
    }
    /// <summary>
    /// Player Joined? sync
    /// </summary>
    ...
    [PunRPC]
    void AddNewKillFeed(string t_Killer, string t_Killed, string t_HowKill, string m_team, int t_GunID, int isHeadShot)
    {
        ...
        {
            KillerColor = isLocalPlayer(t_Killer) ? SelfColor : Color.white;
            KillerColor = isLocalPlayer(t_Killed) ? SelfColor : Color.white;
        }
        else if (setting.m_GameMode != GameMode.FFA)
        {
            if (m_team == Team.Delta.ToString())
            {
                KillerColor = isLocalPlayer(t_Killer) ? SelfColor : bl_GameData.Instance.Team1Color;
                KilledColor = isLocalPlayer(t_Killed) ? SelfColor : bl_GameData.Instance.Team2Color;
            }
            else if (m_team == Team.Recon.ToString())
            {
                KillerColor = isLocalPlayer(t_Killer) ? SelfColor : bl_GameData.Instance.Team2Color;
                KilledColor = isLocalPlayer(t_Killed) ? SelfColor : bl_GameData.Instance.Team1Color;
            }
            else
            {
                KillerColor = isLocalPlayer(t_Killer) ? SelfColor : Color.white;
        ...
        newKillFeed.HeadShot = (isHeadShot == 1) ? true : false;
    ...
    /// <param name="n"></param>
    /// <returns></returns>
    private bool isLocalPlayer(string n)
    {
        bool b = false;
        if (n == LocalName)
        {
