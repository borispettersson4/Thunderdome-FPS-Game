...
public class bl_FirstPersonController : bl_MonoBehaviour
{
    ...
    public bool KeepToCrouch = false;
    [Header("Falling")]
    ...
    private bool fadeInCrouchUIIcon = false;
    ...
    protected override void Awake()
    {
        ...
        m_MouseLook.Init(m_Transform, HeatRoot);
        lastJumpTime = Time.time;
        RunFov = 0;

        fadeInCrouchUIIcon = true;
        bl_UIReferences.Instance.PlayerUI.PlayerStateIcon.CrossFadeAlpha(0, 0.1f, false);
    }
    ...
    public override void OnFixedUpdate()
    {
        ...
        else
        {
            ...
            m_MouseLook.isAiming = bl_Input.GetKey("Aim");
        ...
        if(fadeInCrouchUIIcon)
            bl_UIReferences.Instance.PlayerUI.PlayerStateIcon.CrossFadeAlpha(0, 0.5f, false);
        else
            bl_UIReferences.Instance.PlayerUI.PlayerStateIcon.CrossFadeAlpha(255, 0.1f, false);
    }
        ...
        if (State != PlayerState.Climbing)
        {
            if (m_Input.magnitude > 0)
            {
                if (!bl_UtilityHelper.isMobile)
                {
                    ...
                    else if (State == PlayerState.Running && VerticalInput > 0.5f && (!bl_Input.GetKey("Fire") || !isGrounded))
                    {
                        State = PlayerState.Running;
                    }
    ...
    private void RotateView()
    {
        if (!isClimbing)
        {
            if(!bl_RoomMenu.isGamePaused)
                m_MouseLook.LookRotation(m_Transform, HeatRoot);
        }
        else
        {
            m_MouseLook.LookRotation(m_Transform, m_Ladder.InsertionPoint);
        }
    }
  ...
  /*  private void OnControllerColliderHit(ControllerColliderHit hit)
    ...
    IEnumerator FadeCrouchIcon()
    {
        bl_UIReferences.Instance.PlayerUI.PlayerStateIcon.CrossFadeAlpha(255, 3f, true);
