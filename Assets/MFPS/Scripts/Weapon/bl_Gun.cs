...
public class bl_Gun : bl_GunBase
{
    ...
    void InputUpdate()
    {
        ...
        else
        {
            ...
            if (m_CanFire)
            {
                ...
                if(!FireButton && !FireButtonDown)
                {
                    isHoldingDownFire = false;
                }
            }
            else
            {
...
#if INPUT_MANAGER
        if(bl_Input.Instance.m_InputType == Lovatto.Asset.InputManager.InputType.Xbox || bl_Input.Instance.m_InputType == Lovatto.Asset.InputManager.InputType.Playstation)
        {
    ...
    void MachineGun_Fire()
    {
        ...
        if (FireTypeText.text == bl_GameTexts.FireTypeAuto)
        {
        ...
        else if (FireTypeText.text == bl_GameTexts.FireTypeSingle)
        {
            if ((nextFireTime < time) && !isHoldingDownFire)
            {
                isHoldingDownFire = true;
                StartCoroutine(FireOneShot());
                if (Animat != null)
                {
                    if (isAimed)
    ...
    void ShotGun_Fire()
    {
        ...
        {
            if ((nextFireTime < Time.time) && !isHoldingDownFire)
            {
                do
                {
                    StartCoroutine(FireOneShot());
